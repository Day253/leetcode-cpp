...
\begindot
\item \textbf{删除节点是表头}的情况，输入参数中传入$head$的二级指针，在$for$循环里将其初始化$curr$，然后$entry$就是\fn{*head(*curr)}，我们马上删除它，那么第8行就等效于\fn{*head = (*head)->next}，就是删除表头的实现。
...
\begin{center}
    \includegraphics[width=0.64\textwidth]{two-star-programming.png}\\
