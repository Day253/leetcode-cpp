...
\section{Valid Palindrome (loj125)} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
溢出判断方法：
\begin{Code}
num > INT_MAX / 10 || (num == INT_MAX / 10 && (str[i] - '0') > INT_MAX % 10
\end{Code}
或者
\begin{Code}
...
public:
    string addBinary(string a, string b) {
        string add_result;
        int i = a.length() - 1, j = b.length() - 1, carry = 0, odd = 0;
        while(i >= 0 || j >= 0) {
            if(i < 0) a[i = 0] = '0';
            if(j < 0) b[j = 0] = '0';
            odd = (((a[i] - '0') + (b[j] - '0')) + carry) % 2;
            carry = (((a[i--] - '0') + (b[j--] - '0')) + carry) / 2;
            add_result.insert(0, (odd) ? "1" : "0");
        }
        add_result.insert(0, (carry) ? "1" : "");
        return add_result;
    }
};
\end{Code}
...
\subsubsection{代码}
\begin{Code}
\\ implementing a full adder 高级加法实现方式
class Solution {
public:
...
\section{Longest Common Prefix (loj14)} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
