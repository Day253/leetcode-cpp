...
\item The key intuition to solve the problem is that given a number of stairs n, if we know the number ways to get to the points \fn{[n-1]} and \fn{[n-2]} respectively, denoted as \fn{n1} and \fn{n2} , then the total ways to get to the point \fn{[n]} is \fn{n1 + n2}. Because from the \fn{[n-1]} point, we can take one single step to reach \fn{[n]}. And from the \fn{[n-2]} point, we could take two steps to get there. There is NO overlapping between these two solution sets, because we differ in the final step.
\myenddot
...
\subsubsection{分析}
Same simple algorithm written in every offered language. Variable \fn{a} tells you the number of ways to reach the current step, and \fn{b} tells you the number of ways to reach the next step. So for the situation one step further up, the old \fn{b} becomes the new \fn{a}, and the new \fn{b} is the old \fn{a+b}, since that new step can be reached by climbing 1 step from what \fn{b} represented or 2 steps from what \fn{a} represented.
