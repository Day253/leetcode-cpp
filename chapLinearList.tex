...
如果\fn{A[k/2-1] < B[k/2-1]}，意味着\fn{A[0]}到\fn{A[k/2-1}的肯定在$A \cup B$的top k元素的范围内，换句话说，\fn{A[k/2-1]}不可能大于$A \cup B$的第$k$大元素。留给读者证明(反证，假设不在即可证明。)。
...
\subsubsection{分析}
C++ 12ms 1time traversal and 10x3 mem
        ...
        for (int i = 0; i < 9; i++) {
            ...
            for (int j = 0; j < 9; j++) {
                ...
                if (board[i / 3 * 3 + j / 3][i % 3 * 3 + j % 3] != '.') {
                    if (bank[board[i / 3 * 3 + j / 3][i % 3 * 3 + j % 3] - '0']) {
                        return false;
                    } else {
                        bank[board[i / 3 * 3 + j / 3][i % 3 * 3 + j % 3] - '0'] = true;
                    }
                }
            }
        }
...
\subsubsection{分析}
非常精妙的解法，如果在非9进位，直接返回，在9处返回，一定是从末位进位上去的，因此直接在尾部加一个0，把首位改成1；
...
\subsection{Add Two Numbers (loj2)}
...
\subsection{Reverse Linked List (loj206)}
...
\subsubsection{分析}
xWell, since the \fn{head} pointer may also be modified, we create a \fn{new_head} that points to it to facilitate the reverse process.

For the example list \fn{1 -> 2 -> 3 -> 4 -> 5} in the problem statement, it will become \fn{0 -> 1 -> 2 -> 3 -> 4 -> 5} (we init \fn{new_head -> val} to be \fn{0}). Then we set a pointer\fn{pre} to \fn{new_head} and another \fn{cur} to \fn{head}. Then we keep inserting \fn{cur -> next} after \fn{pre} until \fn{cur} becomes the last node. The code is follows.

\begin{Code}
// LeetCode, Reverse Linked List
// 添加头结点的迭代版，时间复杂度O(n)，空间复杂度O(1)
class Solution {
    public:
    ListNode* reverseList(ListNode* head) {
...
A more concise solution without using the \fn{new_head}. The idea is to reverse one node at a time for the beginning of the list. The rewritten code is as follows.

\begin{Code}
// LeetCode, Reverse Linked List
// 添加头结点的迭代版，此处修改head指针，时间复杂度O(n)，空间复杂度O(1)
class Solution {
    public:
    ListNode* reverseList(ListNode* head) {
        ListNode* pre = NULL;
...
\begin{Code}
// LeetCode, Reverse Linked List
// 递归版本，此处修改head指针，时间复杂度O(n)，空间复杂度O(1)
class Solution {
    public:   
    ListNode* reverseList(ListNode* head) {
...
The basic idea of this recursive solution is to reverse all the following nodes after \fn{head}. Then we need to set \fn{head} to be the final node in the reversed list. We simply set its next node in the original list (\fn{head -> next}) to point to it and sets its \fn{next} to be \fn{NULL}. 
...
\subsection{Reverse Linked List II (loj92)}
...
public:
    ListNode *reverseBetween(ListNode *head, int m, int n) {
        ...
        // prev指向第m结点
        prev = head2->next;
        // cur指向第m+1结点，之后处理模式时将m+1插到m结点之前
        ListNode *cur = prev->next;
        for (int i = m; i < n; ++i) {
            // 头插法
            // 摘下cur结点，将prev连到nxt结点
            prev->next = cur->next;
            // 将cur插到head2和prev之间
            cur->next = head2->next;
            head2->next = cur;
...
\begin{Code}
// LeetCode, Reverse Linked List II
// 另一种迭代版，需要各种检查，时间复杂度O(n)，空间复杂度O(1)
class Solution {
public:
    ListNode* reverseBetween(ListNode* head, int m, int n) {
        if(n==m) return head;
        ...
        ListNode *pre=dummy, *cur, *nxt, *rhead, *rtail;
        int i;
        for(i=0, cur=dummy, nxt=cur->next;
            cur;
            i++, pre=cur, cur=nxt, nxt=cur->next)
        {
             if(i==m-1) rhead=cur;
             if(i==n) rtail=cur;
             if(m<i && i<=n)
...
\subsection{Partition List (loj86)}
...
\subsection{Remove Duplicates from Sorted List (loj83)}
...
\subsection{Delete Node in a Linked List (loj237)}
\label{sec:delete-node-in-a-linked-list}

\subsubsection{描述}
Write a function to delete a node (except the tail) in a singly linked list, given only access to that node.

Supposed the linked list is \code{1 -> 2 -> 3 -> 4} and you are given the third node with value \code{3}, the linked list should become \code{1 -> 2 -> 4} after calling your function.

\subsubsection{分析}
We can't really delete the node, but we can achieve the same effect by instead removing the next node after copying its data into the node that we were asked to delete.
...
\subsection{Remove Linked List Elements (loj203)}
\label{sec:remove-linked-list-elements}
...
Given: \code{1 --> 2 --> 6 --> 3 --> 4 --> 5 --> 6}, \code{val = 6}
...
the trick is using reference (\code{ListNode* \&L})
\subsubsection{代码}
\begin{Code}
class Solution {
public:
...
Another code that can pass all the test with one more variable.
\subsubsection{代码}
\begin{Code}
class Solution {
public:
    ListNode *removeElements(ListNode *head, int val) {
        ListNode *cur = head, *newHead = head, *prev = head;
        while(cur) {
            // 单独处理删除头结点（可能删除多次头结点）
            if(newHead->val == val) {
                prev = newHead->next;
...
\subsubsection{分析}
基本思想，设两个指针$slow,fast$，让$fast$先走$n$步，然后$fast$和$slow$一起走，直到$fast$走到尾节点，删除\fn{slow->next}即可。处理删除头结点需要技巧。
...
\end{Code}
这里\fn{remove_fn}由调用查提供的一个是否删除当前实体结点的函数指针，其会判断删除条件是否成立。这段代码维护了两个节点指针$prev$和$curr$，标准的教科书写法：\textbf{删除当前结点时，需要一个$previous$的指针，并且还要这里还需要做一个边界条件的判断，$curr$是否为链表头}。于是，要删除一个节点（不是表头），只要将前一个节点的$next$指向当前节点的$next$指向的对象，即下一个节点（即：\fn{prev->next = curr->next}），然后释放当前节点。
...
\begindot
\item \textbf{删除节点是表头}的情况，输入参数中传入$head$的二级指针，在$for$循环里将其初始化$curr$，然后$entry$就是\fn{*head}(\fn{*curr})，我们马上删除它，那么第8行就等效于\fn{*head = (*head)->next}，就是删除表头的实现。
\item \textbf{删除节点不是表头}的情况，对于上面的代码，我们可以看到：
    \begin{enumerate}
        \item 如果不删除当前结点，\fn{curr}保存的是当前结点\fn{next}指针的地址。
        \item entry 保存了 \fn{*curr}，这意味着在下一次循环：\fn{entry}就是\fn{prev->next}指针所指向的内存。
        \item 删除结点：\fn{*curr = entry->next}; 于是：\fn{prev->next} 指向了 \fn{entry->next};
    \end{enumerate}
\myenddot
...
\begin{center}
    \includegraphics[width=0.6\textwidth]{two-star-programming.png}\\
    \figcaption{Two star programming}\label{fig:two-star-programming}
\end{center}
...
\subsection{Palindrome Linked List (loj234)}
\label{sec:palindrome-linked-list}
...
\subsection{Swap Nodes in Pairs (loj24)}
...
\subsection{Linked List Cycle (loj141)}
...
\subsection{Linked List Cycle II (loj142)}
...
\subsection{Intersection of Two Linked Lists (loj160)}
\label{sec:intersection-of-two-linked-lists}
...
\textbf{Notes:}
\begindot
\item If the two linked lists have no intersection at all, return \code{null}.
...
\begindot
\item Brute-force solution (O(mn) running time, O(1) memory):
\item Hashset solution (O(n+m) running time, O(n) or O(m) memory):
\item Two pointer solution (O(n+m) running time, O(1) memory):
\myenddot
